...
class Logger:
    ...
    def __init__(
        ...
        self.device = device
        self.git_status_repos = [holosoma_agent.__file__]
    ...
    def init_logging_writer(self) -> None:
        ...
        if self.log_dir is not None and not self.disable_logs:
            ...
            if self.logger_type == "wandb":
                from holosoma_agent.utils.wandb_utils import WandbSummaryWriter
    ...
    def log(
        ...
        if self.writer is not None:
            collection_size = (
                self.cfg.get("num_steps_per_env", 1)
                * self.num_envs
                * self.gpu_world_size
            )
            ...
            for key, value in learning_rate_dict.items():
                self.writer.add_scalar(f"Learning_rate/{key}", value, it)

            # Log noise std
            self.writer.add_scalar(
                "Policy/mean_noise_std", action_std.mean().item(), it
            )
